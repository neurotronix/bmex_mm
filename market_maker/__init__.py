...
def run():
    ...
    parser.add_argument(
        'command', nargs='?',
        help='Instrument symbol on BitMEX or "setup" for first-time config'
    )
    ...
    if args.command is not None and args.command.strip().lower() == 'setup':
        ...
        try:
            ...
            print("Cannot find settings.py. " +
                  "Run 'marketmaker setup' to create project.")
...
def copy_files():
    ...
    if not os.path.isfile(os.path.join(os.getcwd(), 'settings.py')):
        shutil.copyfile(os.path.join(package_base, '_settings_base.py'),
                        'settings.py')

    try:
        shutil.copytree(package_base, os.path.join(os.getcwd(),
                        'market_maker'))
        print(
            "Created marketmaker project." +
            "\n**** \nImportant!!!" +
